...
build: $(EXTRACT_DEPS) $(OBJECTS) libkc_testing.a
...
libkc_testing.a: $(OBJECTS) $(EXTRACT_DEPS) | $(LIB_OUT_DIR)
	ar rcs $(LIB_OUT_DIR)/libkc_testing.a $(OBJECTS) $(shell find $(TMP_OBJ_DIR) -type f -name '*.o')
...
TEST_STATIC_LIBS := kc_testing
TEST_STATIC_LIBS_DIRS := build/lib
